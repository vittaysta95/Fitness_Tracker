...
Google's servers — same Web Speech API, just with a flag set. If your
Chrome supports it, you'll see a banner on the Log page offering to
download the on-device model (one-time, you choose when). Once
installed, a small "On-device (free, offline)" label appears under
the mic button whenever it's actually active.
...
- If your browser doesn't support it (most don't yet), nothing changes
  — no banner, no badge, voice input works exactly as it always has.
  There's no downside to having this code present.
