...
import { listExercises, createExercise, updateExercise, clearAllData, syncNewSeedExercises, addAliasToExercise, findAliasConflict } from '../db.js';
...
import { importExerciseCsv } from '../csvImport.js';
...
export async function renderSettingsPage() {
  ...
  function renderApp() {
    ...
    return `
      <div class="page">
        ...
        <h2 class="section-title">Back-fill from CSV</h2>
        <div class="card" style="margin-bottom: 2rem;">
          <p style="font-size: 0.875rem; margin: 0 0 0.75rem;">
            Upload a CSV in the same format as your data export above to back-fill exercise history. Exercise names must match your library (add any missing exercises in Settings first) — unmatched rows are skipped and listed below.
          </p>
          <input type="file" id="backfill-file-input" accept=".csv" style="display: none;" />
          <button id="backfill-upload-btn" class="btn btn-accent">${icon('upload', 18)} Upload back-fill CSV</button>
          ${
            backfillStatus
              ? `<div style="margin-top: 0.75rem; font-size: 0.875rem;">
                  ${backfillStatus.loading ? '<p class="text-muted">Importing…</p>' : ''}
                  ${
                    backfillStatus.result
                      ? `<p class="text-accent">Imported ${backfillStatus.result.imported} set${backfillStatus.result.imported === 1 ? '' : 's'}.</p>
                         ${
                           backfillStatus.result.skippedRows.length > 0
                             ? `<p style="margin: 0.5rem 0 0.25rem;">${backfillStatus.result.skippedRows.length} row${backfillStatus.result.skippedRows.length === 1 ? '' : 's'} skipped:</p>
                                <ul style="margin: 0; padding-left: 1.1rem; font-size: 0.75rem; color: #facc15;">
                                  ${backfillStatus.result.skippedRows
                                    .slice(0, 20)
                                    .map((s) => `<li>Row ${s.row}: ${escapeHtml(s.reason)}</li>`)
                                    .join('')}
                                  ${backfillStatus.result.skippedRows.length > 20 ? `<li>…and ${backfillStatus.result.skippedRows.length - 20} more.</li>` : ''}
                                </ul>`
                             : ''
                         }`
                      : ''
                  }
                  ${backfillStatus.error ? `<p class="text-intensity">${escapeHtml(backfillStatus.error)}</p>` : ''}
                </div>`
              : ''
          }
        </div>
  ...
  function wireUpEvents() {
    ...
    document.getElementById('backfill-upload-btn')?.addEventListener('click', () => {
      document.getElementById('backfill-file-input')?.click();
    });

    document.getElementById('backfill-file-input')?.addEventListener('change', async (e) => {
      const file = e.target.files[0];
      if (!file) return;
      backfillStatus = { loading: true };
      rerender();
      try {
        const text = await file.text();
        const result = await importExerciseCsv(file.name, text);
        backfillStatus = { loading: false, result };
      } catch (err) {
        backfillStatus = { loading: false, error: 'Import failed. Check the file format.' };
      }
      rerender();
    });
    ...
    document.getElementById('training-save-alias-btn')?.addEventListener('click', async () => {
      ...
      const conflict = await findAliasConflict(trainingTranscript, trainingSelectedExerciseId);
      if (conflict) {
        const proceed = window.confirm(
          `"${trainingTranscript}" is already used for "${conflict.canonical_name}". Saving it here too means voice entries for either exercise could become ambiguous. Save anyway?`
        );
        if (!proceed) return;
      }
