...
function updateActiveNavItem(path) {
  // /trends is kept as a legacy-compatible alias that renders the same
  // combined History/Trends page as /history (see app.js routing) —
  // treat it as the same nav item for highlighting purposes so direct
  // links to the old path don't leave the bottom nav looking unselected.
  const effectivePath = path === '/trends' ? '/history' : path;
  document.querySelectorAll('.nav-item').forEach((el) => {
    if (el.dataset.path === effectivePath) {
