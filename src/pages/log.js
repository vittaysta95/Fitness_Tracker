...
import { listExercises, listSessions, createSession, addSet, getSession, setVolume, formatSetDisplay, getMeta, setMeta, addAliasToExercise, findAliasConflict, getExercisePRs } from '../db.js';
...
export async function renderLogPage() {
  ...
  // Cached PRs for the currently-selected draft exercise — keyed by
  // exercise id so switching the exercise field re-fetches, but
  // re-rendering for unrelated reasons (e.g. typing weight/reps)
  // doesn't refetch every keystroke.
  let draftPRs = null; // { exerciseId, max_weight, max_reps, max_set_volume } | null
  ...
  /**
   * 3 PR stat boxes shown once an exercise is selected in the draft
   * (strength exercises only — cardio has no comparable weight/reps/
   * volume PRs). Values reflect current best from history; highlight
   * classes are toggled live via DOM (see wireUpEvents) rather than
   * a full rerender, so typing doesn't lose input focus/caret.
   */
  function renderPRBoxes(prs) {
    if (!prs) return '';
    return `
      <div class="field-row" id="pr-boxes-row" style="grid-template-columns: 1fr 1fr 1fr; margin-bottom: 1rem;">
        <div class="card" id="pr-box-weight" style="padding: 0.6rem; text-align: center;">
          <p class="text-muted" style="font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.03em; margin: 0 0 2px;">Max Weight</p>
          <p class="font-display" style="font-size: 1.05rem; margin: 0;">${prs.max_weight || '—'}${prs.max_weight ? ' kg' : ''}</p>
        </div>
        <div class="card" id="pr-box-reps" style="padding: 0.6rem; text-align: center;">
          <p class="text-muted" style="font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.03em; margin: 0 0 2px;">Max Reps</p>
          <p class="font-display" style="font-size: 1.05rem; margin: 0;">${prs.max_reps || '—'}</p>
        </div>
        <div class="card" id="pr-box-volume" style="padding: 0.6rem; text-align: center;">
          <p class="text-muted" style="font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.03em; margin: 0 0 2px;">Max Set Volume</p>
          <p class="font-display" style="font-size: 1.05rem; margin: 0;">${prs.max_set_volume || '—'}${prs.max_set_volume ? ' kg' : ''}</p>
        </div>
      </div>`;
  }
  ...
  function renderDraftCard(d) {
    ...
    return `
      <div class="card" style="border: 1px solid var(--surface-high);">
        ...
        ${!d.is_cardio && draftPRs && draftPRs.exerciseId === d.matched_exercise_id ? renderPRBoxes(draftPRs) : ''}
  ...
  function wireUpEvents(rerender) {
    // Live PR highlight on the working set row (index 0) — DOM-only
    // update on every keystroke, no rerender, so focus/caret position
    // is never disturbed while typing.
    function updatePRHighlight() {
      if (!draftPRs) return;
      const weightEl = document.querySelector('.draft-weight-input[data-set-index="0"]');
      const repsEl = document.querySelector('.draft-reps-input[data-set-index="0"]');
      const weight = parseFloat(weightEl?.value);
      const reps = parseInt(repsEl?.value, 10);
      const setVol = Number.isFinite(weight) && Number.isFinite(reps) ? weight * reps : NaN;

      const boxWeight = document.getElementById('pr-box-weight');
      const boxReps = document.getElementById('pr-box-reps');
      const boxVolume = document.getElementById('pr-box-volume');
      boxWeight?.classList.toggle('pr-broken', Number.isFinite(weight) && weight > draftPRs.max_weight);
      boxReps?.classList.toggle('pr-broken', Number.isFinite(reps) && reps > draftPRs.max_reps);
      boxVolume?.classList.toggle('pr-broken', Number.isFinite(setVol) && setVol > draftPRs.max_set_volume);
    }
    document.querySelector('.draft-weight-input[data-set-index="0"]')?.addEventListener('input', updatePRHighlight);
    document.querySelector('.draft-reps-input[data-set-index="0"]')?.addEventListener('input', updatePRHighlight);
    updatePRHighlight();
  ...
  function rerender() {
    ...
    // Fetch PRs whenever the draft's matched exercise changes (and
    // isn't cached already) — fire-and-forget, re-renders again once
    // resolved. Cheap no-op on every other rerender since draftPRs
    // stays cached across keystroke-driven rerenders for the same
    // exercise.
    const currentExerciseId = draft && !draft.is_cardio ? draft.matched_exercise_id : null;
    if (currentExerciseId && draftPRs?.exerciseId !== currentExerciseId) {
      getExercisePRs(currentExerciseId).then((prs) => {
        // Guard against a stale response landing after the person has
        // since switched to a different exercise (or cleared the
        // draft) — only apply it if still relevant.
        const stillCurrent = draft && !draft.is_cardio && draft.matched_exercise_id === currentExerciseId;
        if (!stillCurrent) return;
        draftPRs = { exerciseId: currentExerciseId, ...prs };
        rerender();
      });
