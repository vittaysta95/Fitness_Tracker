...
export async function renderLogPage() {
  ...
  function renderPRBoxes(prs) {
    if (!prs) return '';
    const box = (label, value, unit, id) => `
        <div class="card" id="${id}" style="padding: 0.6rem; text-align: center; display: flex; flex-direction: column; justify-content: center; min-height: 4.25rem; margin-top: 0;">
          <p class="text-muted" style="font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.03em; margin: 0 0 2px;">${label}</p>
          <p class="font-display" style="font-size: 1.05rem; margin: 0;">${value || '—'}${value ? unit : ''}</p>
        </div>`;
    return `
      <div style="border: 1px solid var(--surface-high); border-radius: 16px; padding: 0.875rem; margin-top: 1rem;">
        <p class="section-title" style="margin: 0 0 0.75rem;">Personal bests</p>
        <div class="field-row" id="pr-boxes-row" style="grid-template-columns: 1fr 1fr 1fr; align-items: stretch; margin-bottom: 0;">
          ${box('Max Weight', prs.max_weight, ' kg', 'pr-box-weight')}
          ${box('Max Reps', prs.max_reps, '', 'pr-box-reps')}
          ${box('Max Set Volume', prs.max_set_volume, ' kg', 'pr-box-volume')}
        </div>
      </div>`;
  }
