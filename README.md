...
Two ways to teach the matcher your pronunciation now:

**In Settings:** pick an exercise, say its name the way you naturally
would, and see exactly what the recognizer heard. If it's not already
saved, tap "Commit to memory as alias" to add that exact phrase to the
exercise's alias list.

**In the moment, on the Log page:** when you correct a voice entry's
exercise (e.g. it heard "squat" but you meant "deadlift"), a prompt
appears offering to remember that mis-heard phrase against the
exercise you actually picked — no separate trip to Settings needed.
Only shows up for voice-originated corrections where there's actually
something new to learn (typed manual entries don't trigger it, and it
won't offer to re-save something already known).

**Conflict warning:** if the phrase you're about to save is already
used by a *different* exercise (e.g. "squat" already belongs to Back
Squat), you'll get a warning before saving rather than silently
creating ambiguity between the two. You can still proceed if you're
sure — useful if you've genuinely renamed how you refer to something.

## Voice input for body weight (Biomarkers)

Tap the mic next to the weight field to say your weight instead of
typing it. Handles spelled-out numbers and decimals the same way the
Log page does ("seventy eight point five"), and — since you don't
need to say a unit when stating your own weight — also accepts a bare
number with no "kilos" at all ("seventy eight point five" alone still
works). Like every other voice entry in this app, it fills the field
for you to review, never saves automatically.
